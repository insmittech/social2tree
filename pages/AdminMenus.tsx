...
const UrlInput: React.FC<{
    ...
    const cls = `w-full bg-slate-50 border-none rounded-xl px-3 py-2 text-sm font-bold text-slate-700 focus:ring-2 focus:ring-indigo-100 outline-none ${compact ? 'text-xs' : ''}`;
    return (
        <>
            <select
                value={item.type || 'custom'}
                onChange={(e) => onChange({ type: e.target.value as any, to: '' })}
                className="bg-slate-50 border-none rounded-xl px-3 py-1.5 text-[10px] font-black uppercase tracking-widest text-slate-400 focus:ring-2 focus:ring-indigo-100 outline-none"
            >
...
const SortableMenuItem: React.FC<SortableItemProps> = ({ item, onRemove, onUpdate, allPages, isDashboardTab }) => {
    ...
    return (
        <div ref={setNodeRef} style={style} className={`mb-3 ${isDragging ? 'opacity-90' : ''}`}>
            {/* Parent row */}
            <div className={`flex items-start gap-3 p-4 bg-white border-2 rounded-2xl transition-all ${isDragging ? 'border-indigo-600 shadow-2xl scale-[1.02]' : 'border-slate-100'}`}>
                ...
                <div className="flex-1 flex flex-col gap-3">
                    ...
                    <input
                        ...
                        className="bg-slate-50 border-none rounded-xl px-4 py-2.5 text-sm font-bold text-slate-700 focus:ring-2 focus:ring-indigo-100 outline-none w-full"
                    />
                    {/* URL fields */}
                    ...
                    {isDashboardTab && (
                        <div className="mt-1">
                            <button
                                ...
                                className={`flex items-center gap-2 text-[10px] font-black uppercase tracking-widest transition-colors ${hasChildren ? 'text-indigo-500' : 'text-slate-400 hover:text-slate-600'}`}
                            >
                                        ...
                                        <div key={child.id} className="flex items-start gap-2 p-3 bg-indigo-50/50 rounded-xl border border-indigo-100">
                                            <div className="flex-1 flex flex-col gap-1.5">
                                                <input
                                                    ...
                                                    className="bg-white border-none rounded-lg px-3 py-2 text-xs font-bold text-slate-700 focus:ring-2 focus:ring-indigo-100 outline-none w-full"
                                                />
        ...
        <div className="p-4 sm:p-8 lg:px-12 py-8 overflow-hidden pb-32 lg:pb-12">
            <header className="mb-10 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6">
                <div>
                    ...
                    <p className="text-slate-500 font-bold uppercase tracking-widest text-[10px] mt-1">Draggable Reordering & Nested Sub-Menus</p>
                </div>
