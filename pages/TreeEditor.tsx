...
const TreeEditor: React.FC = () => {
    ...
    if (!activePage) {
        return (
            <div className="p-8 text-center bg-white dark:bg-slate-900/40 rounded-3xl border border-slate-100 dark:border-slate-800/50 m-8 shadow-sm dark:shadow-none">
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 w-full pb-32 lg:pb-12">
            {/* Upper Navigation */}
            <div className="bg-white dark:bg-slate-900/40 border-b border-slate-100 dark:border-slate-800/50 -mx-4 px-4 sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8 py-6 mb-8">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div className="flex items-center gap-5">
                        <button
                            onClick={() => navigate('/dashboard/trees')}
                            className="p-3 bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-slate-700/50 rounded-2xl text-slate-400 dark:text-slate-500 hover:text-slate-900 transition-colors shadow-sm dark:shadow-none active:scale-95"
                        >
                            <ArrowLeft size={20} />
                        </button>
                        <div>
                            <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-0.5">Profile Setting</p>
                            <h1 className="text-2xl font-black text-slate-900 dark:text-white flex items-center gap-2 leading-none">
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        <a
                            ...
                            className="bg-white dark:bg-slate-900/40 text-slate-600 dark:text-slate-300 px-4 py-2.5 rounded-2xl font-black text-xs uppercase tracking-widest border border-slate-200 dark:border-slate-700/50 hover:bg-slate-50 transition-all flex items-center gap-2 shadow-sm dark:shadow-none"
                        >
                            <ExternalLink size={14} /> <span>Live</span>
                        </a>
                        <button
                            onClick={() => navigate(`/dashboard/builder/${id}`)}
                            className="bg-white dark:bg-slate-900/40 text-indigo-600 border border-indigo-100 px-4 py-2.5 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-indigo-50 transition-all flex items-center gap-2 shadow-sm dark:shadow-none"
                        >
                            <PanelsTopLeft size={14} /> <span>Layout</span>
                        </button>
                        <button
                            onClick={() => setActiveTab('share')}
                            className="bg-indigo-600 text-white px-5 py-2.5 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-indigo-700 transition-all flex items-center gap-2 shadow-lg dark:shadow-[0_8px_30px_rgb(0,0,0,0.12)] shadow-indigo-100"
                        >
            ...
            <div className="grid lg:grid-cols-[1fr,340px] gap-12 items-start">
                <div className="space-y-8">
                    {/* Tabs Navigation */}
                    <div className="flex bg-slate-50 dark:bg-[#0b0f19] p-1.5 rounded-[1.5rem] border border-slate-100 dark:border-slate-800/50 overflow-x-auto no-scrollbar whitespace-nowrap">
                        {tabs.map((tab) => (
                            <button
                                key={tab.id}
                                onClick={() => setActiveTab(tab.id as any)}
                                className={`flex-1 min-w-[100px] flex items-center justify-center gap-2 py-3.5 px-4 rounded-[1.25rem] font-black text-[11px] uppercase tracking-widest transition-all ${activeTab === tab.id ? 'bg-indigo-600 text-white shadow-lg dark:shadow-[0_8px_30px_rgb(0,0,0,0.12)] shadow-indigo-100' : 'text-slate-400 dark:text-slate-500 hover:text-slate-900 hover:bg-white'}`}
                            >
                                {tab.icon}
                                {tab.label}
                            </button>
                    ...
                    {/* Tab Panels */}
                    <div className="bg-white dark:bg-slate-900/40 rounded-[2rem] border border-slate-100 dark:border-slate-800/50 p-6 sm:p-10 min-h-[500px] shadow-sm dark:shadow-none">
                        {activeTab === 'links' && (
                            <TreeLinks
                                page={activePage}
