        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 pb-32">
            <header className="mb-12 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                ...
                <button
                    ...
                    className="bg-indigo-600 text-white px-8 py-4 rounded-2xl font-black uppercase tracking-widest hover:bg-indigo-700 transition-all flex items-center gap-3 shadow-xl shadow-indigo-100"
                >
            ...
            <div className="grid lg:grid-cols-2 gap-8">
                {roles.map((role) => (
                    <div
                        key={role.id}
                        className="group bg-white rounded-[2.5rem] border-2 border-slate-100 p-8 hover:border-indigo-200 transition-all hover:shadow-2xl hover:shadow-slate-100 relative overflow-hidden"
                    >
                        ...
                        <div className="flex items-center gap-4 mb-6">
                            ...
                            <div>
                                <h3 className="text-xl font-black text-slate-900 uppercase italic tracking-tight">{role.name}</h3>
                                <p className="text-slate-400 text-xs font-bold uppercase tracking-widest">{role.permissions.length} Permissions Granted</p>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {role.permissions.slice(0, 5).map(p => (
                                <span key={p} className="px-3 py-1.5 bg-slate-50 text-slate-500 rounded-lg text-xs font-black uppercase tracking-widest border border-slate-100">
                                    {p.split(':')[1] || p}
                                </span>
                            ))}
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-white w-full max-w-4xl max-h-[90vh] overflow-hidden rounded-[3.5rem] shadow-2xl flex flex-col animate-in fade-in zoom-in-95 duration-200">
                        <div className="p-10 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                            <div>
                                ...
                                <p className="text-slate-500 text-xs font-bold uppercase tracking-widest mt-1">Configure access for this collection of permissions.</p>
                            </div>
                        ...
                        <div className="flex-1 overflow-y-auto no-scrollbar p-10 space-y-10">
                            <div className="grid md:grid-cols-2 gap-8">
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2">Role Name</label>
                                    <input
                                        type="text"
                                        value={editingRole?.name}
                                        onChange={e => setEditingRole({ ...editingRole, name: e.target.value })}
                                        className="w-full bg-slate-50 border-2 border-slate-50 rounded-2xl px-6 py-4 text-sm font-bold focus:bg-white focus:border-indigo-600 outline-none transition-all shadow-inner"
                            ...
                            <div className="space-y-6">
                                <h3 className="text-xl font-black text-slate-900 uppercase italic tracking-tight border-b-2 border-slate-100 pb-4">Permission Checklist</h3>
                                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                                    {Object.entries(groupedPermissions).map(([module, perms]) => (
                                        <div key={module} className="bg-slate-50/50 rounded-3xl p-6 border border-slate-100">
                                                    ...
                                                    >
                                                        ...
                                                        <div className="pt-0.5">
                                                            {editingRole?.permissions?.includes(p.name) ? (
                                                                ...
                                                                <div className="w-4 h-4 rounded border-2 border-slate-200 mt-0.5"></div>
                                                            )}
                                                        </div>
                                                        <div>
                                                            <p className={`text-xs font-black uppercase tracking-widest ${editingRole?.permissions?.includes(p.name) ? 'text-indigo-600' : 'text-slate-700'}`}>
                                                                {p.name.split(':')[1]}
                                                            </p>
                                                            <p className="text-[9px] text-slate-400 font-bold uppercase tracking-tight mt-0.5">{p.description}</p>
                                                        </div>
                                                    </label>
                                                ))}
                                            </div>
                        ...
                        <div className="p-10 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-4">
                            ...
                            <button
                                ...
                                className="bg-indigo-600 text-white px-10 py-4 rounded-2xl text-sm font-black uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-100 flex items-center gap-3"
                            >
