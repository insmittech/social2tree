        ...
        <div className="p-6 max-w-6xl mx-auto">
            <div className="flex justify-between items-center mb-8">
                <div>
                    ...
                    <p className="text-slate-500 font-medium">Manage pricing, features, and visibility.</p>
                </div>
                <button
            ...
            {isLoading ? (
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {plans.map((plan) => (
                        <div key={plan.id} className={`bg-white rounded-2xl border ${plan.isPopular ? 'border-indigo-600 ring-4 ring-indigo-50' : 'border-slate-100'} p-6 relative group transition-all hover:shadow-xl`}>
                            {!plan.isVisible && (
                                <div className="absolute top-4 right-4 text-slate-400 bg-slate-50 px-2 py-1 rounded-lg text-[10px] font-black uppercase flex items-center gap-1">
                            ...
                            <ul className="space-y-3 mb-8">
                                {plan.features.slice(0, 4).map((f, i) => (
                                    <li key={i} className="flex items-center gap-2 text-slate-600 text-xs font-bold">
                                        <Check size={14} className="text-indigo-600" /> {f}
                                    </li>
                                ))}
                                {plan.features.length > 4 && (
                                    <li className="text-slate-400 text-[10px] font-bold uppercase pl-6">+{plan.features.length - 4} more</li>
                                )}
                            </ul>
                            ...
                            <div className="flex gap-2 border-t border-slate-50 pt-4 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button
                                    ...
                                    className="flex-1 bg-slate-50 text-slate-600 py-2 rounded-lg font-bold text-xs hover:bg-indigo-50 hover:text-indigo-600 transition-all flex items-center justify-center gap-2"
                                >
                                    <Edit2 size={14} /> Edit
                                </button>
                                <button
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
                    <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
                        <div className="px-8 py-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                            <h2 className="text-xl font-black text-slate-900">{currentPlan.id ? 'Edit Plan' : 'New Subscription Plan'}</h2>
                            <button onClick={() => setIsEditing(false)} className="p-2 text-slate-400 hover:text-slate-900 rounded-xl transition-colors"><X size={20} /></button>
                        </div>
                        ...
                        <div className="p-8 max-h-[70vh] overflow-y-auto no-scrollbar">
                            <div className="grid md:grid-cols-2 gap-6 mb-8">
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-slate-50 border-none rounded-xl px-4 py-3 text-sm font-bold text-slate-900 focus:ring-2 focus:ring-indigo-600 transition-all"
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Yearly Price ($)</label>
                                        <input
                                            type="number"
                            ...
                            <div className="mb-8">
                                ...
                                <textarea
                                    ...
                                    className="w-full bg-slate-50 border-none rounded-xl px-4 py-3 text-sm font-bold text-slate-900 focus:ring-2 focus:ring-indigo-600 transition-all min-h-[80px]"
                            ...
                            <div className="mb-8">
                                <div className="flex justify-between items-center mb-4">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Plan Features</label>
                                        ...
                                        <div key={idx} className="flex gap-2">
                                            <div className="flex-1 relative">
                                                ...
                                                <input
                                                    ...
                                                    className="w-full bg-slate-50 border-none rounded-xl pl-10 pr-4 py-2.5 text-xs font-bold text-slate-900 focus:ring-2 focus:ring-indigo-600 transition-all"
                            ...
                            <div className="grid grid-cols-3 gap-4">
                                <div className="bg-slate-50 p-4 rounded-2xl flex items-center justify-between">
                                    <span className="text-[10px] font-black text-slate-500 uppercase">Popular</span>
                                    <button
                                    ...
                                    >
                                        <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-transform ${currentPlan.isPopular ? 'translate-x-6' : 'translate-x-1'}`} />
                                    </button>
                                </div>
                                <div className="bg-slate-50 p-4 rounded-2xl flex items-center justify-between">
                                    <span className="text-[10px] font-black text-slate-500 uppercase">Visible</span>
                                    <button
                                    ...
                                    >
                                        <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-transform ${currentPlan.isVisible ? 'translate-x-6' : 'translate-x-1'}`} />
                                    </button>
                                </div>
                                <div>
                                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 pl-1">Sort Order</label>
                                    <input
                                        type="number"
                                        value={currentPlan.sortOrder}
                                        onChange={e => setCurrentPlan({ ...currentPlan, sortOrder: parseInt(e.target.value) })}
                                        className="w-full bg-slate-50 border-none rounded-xl px-4 py-2 text-sm font-bold text-slate-900 focus:ring-2 focus:ring-indigo-600 transition-all"
                                    />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="p-8 bg-slate-50/50 border-t border-slate-100 flex justify-end gap-3">
                            <button
                                ...
                                className="px-6 py-2.5 rounded-xl font-bold text-sm text-slate-500 hover:text-slate-900 transition-colors"
                            >
                                Cancel
                            </button>
                            <button
