      ...
      <div className="flex justify-around items-center h-16">
        {links.map((link) => (
          <NavLink
            key={link.to}
            to={link.to}
            end={link.to === '/dashboard/links' || link.to === '/admin'}
            className={({ isActive }) =>
          ...
          >
            ...
            <div className={`w-1 h-1 rounded-full bg-indigo-600 transition-all duration-300 ${location.pathname === link.to || (link.to === '/dashboard' && location.pathname === '/dashboard') ? 'opacity-100' : 'opacity-0'
              }`} />
          </NavLink>
        ))}
      </div>
