    ...
    return (
        <div
            className={`group relative rounded-[2.5rem] p-8 border-2 transition-all duration-500 hover:-translate-y-2 flex flex-col h-full
                ${isCurrent
                    ? 'border-indigo-600 bg-indigo-600 text-white shadow-[0_20px_50px_rgba(79,70,229,0.3)]'
                    : plan.isPopular
                        ? 'border-indigo-100 bg-white dark:bg-slate-900/40 shadow-xl dark:shadow-none hover:shadow-2xl dark:hover:border-indigo-500/50'
                        : 'border-slate-50 dark:border-slate-800/50 bg-white/50 dark:bg-slate-950/30 backdrop-blur-sm'
                }`}
        >
            ...
            {/* Title & Description */}
            <div className="mb-6">
                <h3 className={`text-2xl font-black uppercase tracking-tight mb-2 ${isCurrent ? 'text-white' : 'text-slate-900 dark:text-white'}`}>
                    {plan.name}
                </h3>
                <p className={`text-sm font-bold leading-relaxed ${isCurrent ? 'text-indigo-50' : 'text-slate-400 dark:text-slate-500'}`}>
                    {plan.description}
                </p>
            </div>
            ...
            <div className="mb-8">
                <div className="flex items-baseline gap-1">
                    ...
                    <span className={`text-lg font-black ${isCurrent ? 'text-indigo-100' : 'text-slate-400 dark:text-slate-500'}`}>
                        {price % 1 !== 0 && `.${(price % 1).toFixed(2).split('.')[1]}`}
                        {billedLabel}
                    </span>
                </div>
                {billingCycle === 'yearly' && price > 0 && (
                    <p className={`text-[10px] font-black uppercase mt-2 tracking-widest ${isCurrent ? 'text-indigo-200' : 'text-emerald-500'}`}>
            ...
            <ul className="space-y-4 mb-10 flex-1">
                {plan.features.map((feature, i) => (
                    <li key={i} className="flex items-start gap-4">
                        ...
                        <span className={`text-sm font-bold ${isCurrent ? 'text-indigo-50' : 'text-slate-600 dark:text-slate-300'}`}>
                            {feature}
                        </span>
                    </li>
                ))}
